import { getContentStore, getJSON, setJSON, unitTemplateKey } from './_shared/blobs.mts';
...
import { UnitTemplateSchema, type UnitTemplate } from '../../src/schemas';
import {
  applyParsedStatus,
...
export default async function handler(request: Request, context: FunctionContext): Promise<Response> {
  ...
  if (request.method === 'DELETE') {
    if (!earlyId) {
      return withCors(errorResponse(404, 'not_found', 'Unit template not found'), request, env);
    }
    return handlePermanentDelete(request, context, 'unit_template');
  }
  ...
  if (!id) {
    return withCors(errorResponse(400, 'validation_error', 'Unit template id is required'), request, env);
  }

  const store = getContentStore();
  const raw = await getJSON<UnitTemplate>(store, unitTemplateKey(id));
  if (!raw) {
    return withCors(errorResponse(404, 'not_found', 'Unit template not found'), request, env);
  }
  ...
  if (!existing.success) {
    return withCors(errorResponse(500, 'invalid_data', 'Stored unit template is invalid'), request, env);
  }
