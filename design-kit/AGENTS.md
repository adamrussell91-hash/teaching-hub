...
Hub marks: also read `ICONS.md`. The blue/orange site tile is **favicon only** — never on the page.
...
| Page wash + card glass (not `data-hub` overlays) | One supporting line (if the hub still has one) |
...
No supporting line, purpose copy, privacy notes, extra rows, hub tile (`.sign-in__mark` / `.hub-mark`), or hub-only login CSS. The card is haze + type + field.
...
- Hub mark: **do not render** `.hub-mark` or `.sign-in__mark`. Favicon only. See `ICONS.md`.
...
3. Sign out / refresh → copy `snippets/hub-utilities.html` into `.page-header__actions` (canvas top-right). Keep existing ids/data attributes if tests rely on them; change the markup to `.hub-icon-btn`. Do **not** add a `.hub-mark` tile. Favicon may keep the tile. Drop `.sign-in__supporting` and any login purpose copy.
...
5. Favicon may use that hub’s tile from `icons/`. Do not render `.hub-mark` or `.sign-in__mark`.
